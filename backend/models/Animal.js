...
class Animal{
    constructor({animal_id, name, type, group_behaviour, population_in_uk, capture_points, pack_bonus_mult, description, fun_fact, profile_picture}){
    ...
    static async create(data) {
        const {name, type, group_behaviour, population_in_uk, capture_points, pack_bonus_mult, description, fun_fact, profile_picture } = data;
        const alreadyExists = await db.query("SELECT * FROM animals WHERE name = $1", [name]);
        if (alreadyExists.rows.length !== 0) {
            throw new Error('Animal already exists.')
        }
        let response = await db.query("INSERT INTO animals (name, type, group_behaviour, population_in_uk, capture_points, pack_bonus_mult, description, fun_fact, profile_picture) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING animal_id;", [name, type, group_behaviour, population_in_uk, capture_points, pack_bonus_mult, description, fun_fact, profile_picture]);
